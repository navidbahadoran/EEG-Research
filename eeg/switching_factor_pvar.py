...
@dataclass
class SwitchingFactorPVARResult:
...
class SwitchingFactorPVAR:
    """
    Common-regime switching factor-PVAR.

    Model:
        y_{i,t} = c_{z_t} + A_{z_t} y_{i,t-1} + B_{z_t} x_{i,t} + Lambda_{z_t} f_t + u_{i,t}

    where:
        z_t is common across units and follows a Markov chain,
        f_t is a common latent factor at time t.
    ...
    def __init__(
        ...
        self.r = int(factor_rank)
        self.ridge_w = float(ridge_w)
        self.ridge_f = float(ridge_f)
        ...
        self.Lambda_: Optional[np.ndarray] = None    # (K, d, r)
        self.f_: Optional[np.ndarray] = None         # (Tm_train, r)
    ...
    def _initialize_params(self, Ylag: np.ndarray, Ycur: np.ndarray, Xcur: np.ndarray) -> None:
        ...
        self.Lambda_ = 0.05 * self.rng_.standard_normal((self.K, d, self.r))
        self.f_ = 0.05 * self.rng_.standard_normal((Tm, self.r))
        ...
        # More persistent initial regime dynamics
        # self.Pi_ = 0.80 * np.eye(self.K) + 0.20 / self.K
        # self.Pi_ = _safe_normalize_rows(self.Pi_)
        # self.pi0_ = np.full(self.K, 1.0 / self.K)

        # pooled initialization for c, A, B
        Z = self._build_design(Ylag, Xcur)                    # (N, Tm, 1+d+p)
        Z2 = Z.reshape(-1, Z.shape[-1])
        Y2 = Ycur.reshape(-1, d)
        W_pooled = _ridge_solve(Z2, Y2, self.ridge_w)        # (1+d+p, d)

        self.c_[:] = W_pooled[0:1, :]
        A0 = W_pooled[1:1 + d, :]                            # (d, d)
        B0 = W_pooled[1 + d:, :]                             # (p, d)
    ...
    def _compute_mu_all_regimes(
        ...
        Ylag: np.ndarray,
        Xcur: np.ndarray,
        f_override: Optional[np.ndarray] = None,
    ) -> np.ndarray:
        """
        ...
        Inputs:
            Ylag: (N, Tm, d)
            Xcur: (N, Tm, p)
            f_override: optional factor path with shape (Tm, r)
        ...
        f_used = self.f_ if f_override is None else np.asarray(f_override, dtype=float)
        if f_used.shape[0] != Tm:
            raise ValueError(
                f"Factor path length mismatch: f_used has length {f_used.shape[0]}, but Tm={Tm}."
            )
        ...
        for k in range(self.K):
            ...
            fac_term = f_used @ self.Lambda_[k].T             # (Tm, d)
            fac_term = np.broadcast_to(fac_term[None, :, :], (N, Tm, d))

            mu[k] = self.c_[k][None, None, :] + ar_term + cov_term + fac_term

        return mu

    def _log_emission_probs(
        self,
        Ycur: np.ndarray,
        mu: np.ndarray,
    ) -> np.ndarray:
        """
        Common-regime likelihood across units.
        ...
        N, Tm, d = Ycur.shape
        loglik = np.zeros((Tm, self.K), dtype=float)
    ...
    def _forward_backward(
        ...
        """
        # assert self.Pi_ is not None
        # assert self.pi0_ is not None
        if self.Pi_ is None:
           raise RuntimeError("Pi_ is None inside _forward_backward; initialization failed.")
        if self.pi0_ is None:
            raise RuntimeError("pi0_ is None inside _forward_backward; initialization failed.")
    ...
    def _update_markov(self, gamma: np.ndarray, xi: np.ndarray) -> None:
        self.pi0_ = gamma[0]
        self.Pi_ = _safe_normalize_rows(np.sum(xi, axis=0))
    ...
    def _update_regression_params(
        ...
        """
        Update c_k, A_k, B_k regime by regime.

        gamma: (Tm, K) is common across units, so weights are repeated across i.
        """
        assert self.c_ is not None
        assert self.A_ is not None
        assert self.B_ is not None
        ...
        fac = np.zeros((self.K, Tm, d), dtype=float)
        for k in range(self.K):
            fac[k] = self.f_ @ self.Lambda_[k].T

        Z = self._build_design(Ylag, Xcur)
        Z2 = Z.reshape(-1, 1 + d + p)

        for k in range(self.K):
            Y_adj = Ycur - fac[k][None, :, :]
            Y2 = Y_adj.reshape(-1, d)

            w = np.repeat(gamma[:, k], N)
            w = np.clip(w, 1e-12, None)
            sw = np.sqrt(w)[:, None]
            ...
            Wk = _ridge_solve(Zw, Yw, self.ridge_w)
    ...
    def _update_common_factors(
        ...
        """
        Update f_t one time point at a time:

            min_f sum_k gamma[t,k] sum_i || y_it - c_k - A_k y_i,t-1 - B_k x_it - Lambda_k f ||^2
        """
        assert self.c_ is not None
        assert self.A_ is not None
        assert self.B_ is not None
        ...
        for t in range(Tm):
            lhs = np.zeros((self.r, self.r), dtype=float)
            rhs = np.zeros(self.r, dtype=float)

            for k in range(self.K):
                g = float(np.clip(gamma[t, k], 1e-10, None))
                Lam = self.Lambda_[k]                          # (d, r)
                ...
                resid = Ycur[:, t, :] - base                  # (N, d)
                resid_sum = np.sum(resid, axis=0)             # (d,)

                lhs += g * N * (Lam.T @ Lam)
                rhs += g * (Lam.T @ resid_sum)

            lhs += self.ridge_f * np.eye(self.r)
            self.f_[t] = np.linalg.solve(lhs, rhs)
    ...
    def _update_loadings(
        ...
        """
        Update Lambda_k regime by regime from:

            Ycur - c_k - A_k Ylag - B_k Xcur = Lambda_k f_t + noise
        """
        assert self.c_ is not None
        assert self.A_ is not None
        assert self.B_ is not None
        ...
        for k in range(self.K):
            ...
            for t in range(Tm):
                ...
                resid = Ycur[:, t, :] - base
                R[t] = np.mean(resid, axis=0)

            w = np.clip(gamma[:, k], 1e-12, None)
            sw = np.sqrt(w)[:, None]
            Fw = self.f_ * sw
            Rw = R * sw

            Lam_T = _ridge_solve(Fw, Rw, self.ridge_w)        # (r, d)
            self.Lambda_[k] = Lam_T.T                         # (d, r)

    # ------------------------------------------------------------------
    # fit / predict
    # ------------------------------------------------------------------
    def fit(self, Y_list: List[np.ndarray], X_list: List[np.ndarray]) -> "SwitchingFactorPVAR":
        _, _, Ylag, Ycur, Xcur = self._prepare_tensors(Y_list, X_list)
        self._initialize_params(Ylag, Ycur, Xcur)
        self.loss_history_ = []
        # safety initialization in case local file drifted or state was reset
        if self.Pi_ is None:
            self.Pi_ = np.full((self.K, self.K), 1.0 / self.K)
            self.Pi_ += 0.25 * np.eye(self.K)
            self.Pi_ = _safe_normalize_rows(self.Pi_)
        ...
        for _ in range(self.max_iter):
            mu = self._compute_mu_all_regimes(Ylag, Xcur)
            loglik_tk = self._log_emission_probs(Ycur, mu)
            gamma, xi, nll = self._forward_backward(loglik_tk)

            self._update_markov(gamma, xi)
            self._update_regression_params(Ylag, Ycur, Xcur, gamma)
            self._update_common_factors(Ylag, Ycur, Xcur, gamma)
            self._update_loadings(Ylag, Ycur, Xcur, gamma)
    ...
    def _predict_mean(
        ...
        """
        Mixture prediction with regime weights gamma.

        Returns:
            yhat: (N, Tm, d)
        """
        mu = self._compute_mu_all_regimes(Ylag, Xcur, f_override=f_override)
        yhat = np.zeros((Ylag.shape[0], Ylag.shape[1], Ylag.shape[2]), dtype=float)

        for k in range(self.K):
            yhat += gamma[:, k][None, :, None] * mu[k]
    ...
    def forecast_and_score(
        ...
        # out-of-sample starts at original time index train_end
        ...
        T_oof = Ycur_oof.shape[1]

        # naive factor forecast: carry forward last training factor
        f_last = self.f_[-1]
        f_oof = np.repeat(f_last[None, :], T_oof, axis=0)

        mu_oof = self._compute_mu_all_regimes(Ylag_oof, Xcur_oof, f_override=f_oof)
        loglik_tk = self._log_emission_probs(Ycur_oof, mu_oof)
        gamma_oof, _, _ = self._forward_backward(loglik_tk)

        yhat_oof = self._predict_mean(Ylag_oof, Xcur_oof, gamma_oof, f_override=f_oof)
        ...
        # combined coefficient block: (K, 1+d+p, d)
        ...
        return SwitchingFactorPVARResult(
            metrics={
                "mse": mse,
                "rmse": rmse,
                "r2": r2,
