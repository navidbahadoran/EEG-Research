...
def run_unit_switching_pvar(
    ...
    print("Fitting unit-specific switching PVAR ...")
    ...
    metrics_path = out_dir / f"unit_switching_pvar_k{K}_metrics.csv"
    save_metrics_row(metrics_path, {
        "mode": f"unit_switching_pvar_k{K}",
        "N_units": len(units),
        "common_T": common_T,
        "K": K,
        "p": p,
    ...
    artifacts_path = out_dir / f"unit_switching_pvar_k{K}_artifacts.npz"
    np.savez_compressed(
        artifacts_path,
        units=units,
        tasks=tasks,
