...
def run_switching_factor_pvar(
    ...
    print("Fitting switching factor-PVAR ...")
    ...
    metrics_path = out_dir / f"switching_factor_pvar_k{K}_metrics.csv"
    save_metrics_row(metrics_path, {
        "mode": f"switching_factor_pvar_k{K}",
        "N_units": len(units),
        "common_T": common_T,
        "K": K,
        "factor_rank": factor_rank,
    ...
    artifacts_path = out_dir / f"switching_factor_pvar_k{K}_artifacts.npz"
    np.savez_compressed(
        artifacts_path,
        units=units,
        tasks=tasks,
